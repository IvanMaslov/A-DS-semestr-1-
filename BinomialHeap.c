...
//INV: lhs & rhs are from root list: their leftBro == rightBro == themself and pred == 0
//INV1: lhs & rhs has equal rank
...
struct vert* get_min_heaps(struct heaps* arg) {
	...
	for (int i = 0; i < size_; ++i)
		if (arg->root[i] != 0 && (ans == -1 || arg->root[ans]->value > arg->root[i]->value
			|| arg->root[ans]->value == arg->root[i]->value && arg->root[ans]->index > arg->root[i]->index))
			ans = i;
	//assert(ans >= 0);
	return arg->root[ans];
}
